...
_TORCHSERVE_HANDLER_KEY = "torchserve_handler"
_REQUIREMENTS_FILE_KEY = "requirements_file"
_TORCHSERVE_BUILTIN_HANDLERS = {
    "image_classifier",
    "image_segmenter",
    "object_detector",
    "text_classifier",
...
# TODO (chiragjn):
#   - Add support for state dict and an argument to take in model file explicitly
#   - Simplify signature to only have one way of providing additional python dependencies
#   - Simplify signature to only have one way of providing extra files
# Comments on args:
# We'll enable model_file if we accept any arbitrary state_dict in `pytorch_model` this is also required in case model
# is not defined in `__main__` module and is imported because cloudpickle would only save them as references,
# To pickle imported modules by value see `cloudpickle.register_pickle_by_value`
#     model_file=None,
# only one of the following three is allowed
#     conda_env=None,  # conda_env has the highest preference
#     pip_requirements=None,  # if this is given, then no auto inference is done and these are used as is
#     extra_pip_requirements=None,  # if this is given, requirements are inferred and these are added
# changing pickle_module should be discouraged, ideally we should remove this from args
#     pickle_module=None,
# arguments that will be removed, discourage using these
...
def log_model(
    ...
    **kwargs,
    ...
    return Model.log(
        ...
        torchserve_handler=torchserve_handler,
        conda_env=conda_env,
...
def _save_torchserve_handler(torchserve_handler, path, torchserve_artifacts_config):
    if not torchserve_handler:
        torchserve_artifacts_config[_TORCHSERVE_HANDLER_KEY] = None
        return

    if not isinstance(torchserve_handler, str):
        raise TypeError(
            "`torchserve_handler` should be Path to a file or one of the torchserve built-in handler names"
        )

    if torchserve_handler in _TORCHSERVE_BUILTIN_HANDLERS:
        torchserve_artifacts_config[_TORCHSERVE_HANDLER_KEY] = {"name": torchserve_handler}
    else:
        _logger.info(
            f"torchserve_handler={torchserve_handler!r} not recognised as one of the built-in handlers."
            f"Assuming the handler is a path to a file and copying it"
        )
        with TempDir() as tmp_handler_dir:
            _download_artifact_from_uri(
                artifact_uri=torchserve_handler, output_path=tmp_handler_dir.path()
            )
            rel_path = os.path.basename(torchserve_handler)
            torchserve_artifacts_config[_TORCHSERVE_HANDLER_KEY] = {"path": rel_path}
            shutil.move(tmp_handler_dir.path(rel_path), path)
...
def _save_environment(
    ...
    if requirements_file:
        # TODO (chiragjn): Remove this block
        warnings.warn(
            "`requirements_file` has been deprecated. Please use `pip_requirements` instead.",
            FutureWarning,
            stacklevel=2,
    ...
    if pip_constraints:
        ...
        torchserve_artifacts_config[_EXTRA_FILES_KEY].append({"path": _CONSTRAINTS_FILE_NAME})

    # Save `requirements.txt`
    if not requirements_file:
        # TODO (chiragjn): Always save this
        write_to(os.path.join(path, _REQUIREMENTS_FILE_NAME), "\n".join(pip_requirements))
        torchserve_artifacts_config[_REQUIREMENTS_FILE_KEY] = {"path": _REQUIREMENTS_FILE_NAME}
...
def save_model(
    ...
    **kwargs,
    ...
    if isinstance(pytorch_model, torch.jit.ScriptModule):
        ...
        # TODO (chiragjn): If we use cloudpickle 2.x, we can register pytorch_model.__module__ to be
        #   pickled by reference. However we need to test how much arbitrarily nesting can this work with and
        #   what happens when there are relative imports etc
        torch.save(pytorch_model, model_path, pickle_module=pickle_module, **kwargs)

    torchserve_artifacts_config = {
        _EXTRA_FILES_KEY: [],
        _TORCHSERVE_HANDLER_KEY: None,
    }

    _save_extra_files(
        extra_files=extra_files, path=path, torchserve_artifacts_config=torchserve_artifacts_config
    )
    _save_torchserve_handler(
        torchserve_handler=torchserve_handler,
        path=path,
        torchserve_artifacts_config=torchserve_artifacts_config,
    )
    _save_environment(
        conda_env=conda_env,
        pip_requirements=pip_requirements,

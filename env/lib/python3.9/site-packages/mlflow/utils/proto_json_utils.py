...
def get_field_if_set(request_message, field_name: str, default: Any) -> Any:
    # Note: HasField returns False even if field is set but with a None value
    if request_message.HasField(field_name):
        return getattr(request_message, field_name)
    return default
